...
var callbacks = {};
var callMethodChannel = new BroadcastChannel("callMethod");
var callMethodResponseChannel = new BroadcastChannel(`callMethod:response`);
...
function callMethod(name, args, options) {
...
var callFunctionChannel = new BroadcastChannel("callFunction");
var callFunctionResponseChannel = new BroadcastChannel(`callFunction:response`);
...
var MemFS = class {
  constructor() {
    this._onDidRunOperation = new vscode.EventEmitter();
    this.onDidRunOperation = this._onDidRunOperation.event;
    this._emitter = new vscode.EventEmitter();
    this.onDidChangeFile = this._emitter.event;
    this.pasteOperations = 0;
  ...
  findFile(path) {
    ...
    return current.entries.get(parts[parts.length - 1]);
  }
  ...
  async readDirectory(uri) {
    ...
    try {
      var results = result.data.items.map((item) => [item.path, item.type === "dir" ? vscode.FileType.Directory : vscode.FileType.File]);
    } catch (err) {
      console.error(err);
    }
    return results;
...
function activate(context) {
  ...
  callFunction("getWorkspaces", []).then((result) => {
    console.log("getWorkspaces", result);
    let workspacesCount = vscode2.workspace.workspaceFolders?.length || 0;
    vscode2.workspace.updateWorkspaceFolders(0, workspacesCount, ...result.map((workspace3) => ({ uri: vscode2.Uri.parse("memfs:/" + workspace3.guid + "/"), name: workspace3.organizationName })));
  });
